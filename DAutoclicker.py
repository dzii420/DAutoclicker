...
def update_countdown_display():
    ...
    if not is_running:
        countdown_label.config(text="Next click in: --:--:--.--") # Updated format
        return
    ...
    if remaining_ms > 0:
        countdown_update_id = root.after(100, update_countdown_display) # Update more frequently for milliseconds
    else:
...
def autoclicker_loop():
    ...
    while is_running:
        ...
        root.after(0, update_countdown_display) # Start/restart countdown display
...
def create_gui():
    ...
    root.geometry("500x750") # Increased height slightly for new sections
